...
if __name__ == "__main__":
	for firstPara in range(FIRST_RANGE_START,FIRST_RANGE_END,FIRST_RANGE_STEP):
		for secondPara in range(SECOND_RANGE_START,SECOND_RANGE_END,SECOND_RANGE_STEP):
			...
			clusterResultFile = "cluster_result/%s_%s_%s_%s_%s_%s.txt" % (DIVISIOR,EPS_VALUE,MIN_SAMPLE,K_VALUE,firstPara,secondPara)
				...
				for i in range(length):
					for j in range(i,length):
						if i == j:
							...
							timeDiff[j][i] = timeDiff[i][j] = math.sqrt((timeDiffTrans[i][j] * firstPara + timeDiffStretch[i][j] * secondPara * K_VALUE)/DIVISIOR)/len(frequentStr)
